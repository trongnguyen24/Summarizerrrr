import { summaryState, summarizeSelectedText, resetDisplayState, updateVideoActiveStates, fetchAndSummarize, updateActiveCourseTab, updateActiveYouTubeTab } from '../stores/summaryStore.svelte.js';
...
function handleBackgroundMessage(request) {
  switch (request.action) {
    case 'tabUpdated':
    case 'currentTabInfo':
      console.log(`[messageHandler.js] Received ${request.action} message:`, request);
      ...
      break;
    case 'displaySummary':
      console.log('[messageHandler.js] Received displaySummary message:', request);
      summaryState.summary = request.summary;
      summaryState.error = request.error ? request.summary : null;
      break;
    case 'udemyTranscriptAvailable':
      console.log('[messageHandler.js] Received udemyTranscriptAvailable message:', request);
    ...
    case 'courseraContentAvailable':
      console.log('[messageHandler.js] Received courseraContentAvailable message:', request);
      summaryState.isCourseVideoActive = true;
      summaryState.lastSummaryTypeDisplayed = 'course';
      updateActiveCourseTab('courseSummary');
      break;
    case 'summarizeSelectedText':
      console.log('[messageHandler.js] Received summarizeSelectedText message:', request);
      resetDisplayState();
      summaryState.lastSummaryTypeDisplayed = 'selectedText';
      summarizeSelectedText(request.selectedText);
      break;
    case 'summarizeCurrentPage':
      console.log('[messageHandler.js] Received summarizeCurrentPage message:', request);
      resetDisplayState();
...
export function setupMessageListener() {
  ...
  return () => {
    ...
    console.log('[messageHandler.js] Removed fallback runtime.onMessage listener.');
  };
}
