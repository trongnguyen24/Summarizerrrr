...
<div class="flex flex-row items-center gap-0.5">
  <Switch.Root
    ...
    class="focus-visible:ring-primary group overflow-hidden relative  text-text-secondary flex items-center focus-visible:ring-offset-background pl-10 pr-2  transition-colors focus-visible:outline-hidden w-full h-9  cursor-pointer  focus-visible:ring-1 focus-visible:ring-offset-1 data-[state=checked]:border-border data-[state=checked]:text-white"
  >
    <div
    ...
    />
    <Label.Root
      class="cursor-pointer overflow-hidden font-black line-clamp-1 text-text-secondary select-none {checked
        ? ' !text-text-primary '
        : ''}"
      for={id}
    >
      {name}: {checked ? 'Active' : 'Deactivate'}
    </Label.Root>
  </Switch.Root>
</div>
