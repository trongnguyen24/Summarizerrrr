...
1. Use Gemini Basic mode
2. Trigger summarization
3. Verify "Using model: gemini-2.5-flash" appears during loading
4. Verify status disappears after successful completion

### Test 2: Fallback Triggered
1. Use Gemini Basic mode
2. Trigger overload error (or wait for natural overload)
3. Verify "Using model: gemini-2.5-flash" appears initially
4. Verify "Model switched: gemini-2.5-flash → gemini-2.0-flash" appears after fallback
5. Verify fallback status persists after completion

### Test 3: Multiple Fallbacks
1. Trigger overload on first two models
2. Verify status updates through entire chain
3. Verify final status shows correct original and current model

### Test 4: Non-Gemini Providers
1. Use OpenAI or Claude
2. Verify no model status is shown (auto-fallback not enabled)

### Test 5: Advanced Mode
1. Use Gemini Advanced mode
2. Verify no model status is shown (auto-fallback not enabled)
...
4. **Settings Toggle**
   - Allow users to hide/show model status
   - Preference saved in settings

5. **Other Provider Fallback**
