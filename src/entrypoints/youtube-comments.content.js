...
export default defineContentScript({
  ...
  async main() {
    ...
    if (window.youtubeCommentsListenerAdded) return
    window.youtubeCommentsListenerAdded = true

    console.log('[YouTubeComments] Content script loaded')
    ...
    browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      if (message.action === 'fetchYouTubeComments') {
        const maxComments = message.maxComments || 50
        const maxRepliesPerComment = message.maxRepliesPerComment || 10

        fetchYouTubeComments(maxComments, maxRepliesPerComment)
          .then((result) => {
            sendResponse(result)
          })
          .catch((error) => {
    ...
    console.log('[YouTubeComments] Ready')
  },
})
